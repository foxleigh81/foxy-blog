...
test.describe('Tag Page', () => {
  test('displays tag information correctly', async ({ page }) => {
    await (page as CustomPage).navigateAndWait('/tag/vr');

    // Check common elements
    await (page as CustomPage).checkCommonPageElements();

    // Check for tag title (h1 or similar heading)
    await page.waitForSelector('h1', { timeout: 30000 });
  });

  test('displays tagged posts', async ({ page }) => {
    await (page as CustomPage).navigateAndWait('/tag/career');
